...
/// <summary>
/// Modified by Madison Marzano
/// </summary>


[RequireComponent( typeof(MeshFilter), typeof(MeshRenderer) )]
public class ProceduralCapsuleMM : MonoBehaviour 
{
	#if UNITY_EDITOR
	...
	void GenerateMesh() 
	{
		...
		int vertexCount = (points * (points + 1)) + (ringCount * points); //--MM
		...
		// TODO -- Center Cylinder
		for ( int y = 0; y < ringCount; y++ )
		{
			for ( int x = 0; x < points; x ++)
			{
				vertices[ ind ] = new Vector3( pX[x], pY[top - 1], pZ[x] ) * radius;
				vertices[ ind ].y = (y / yOff) - vertices[ ind ].y ;
		...
		int[] triangles = new int[ ( segments * (segments + ringCount + 1) * 2 * 3 ) ];
		
		for ( int y = 0, t = 0; y < segments + ringCount + 1; y ++ ) 
		{
